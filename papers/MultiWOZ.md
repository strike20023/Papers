...
## 贡献亮点
- 大规模、多领域、全标注的任务型对话数据集，显著提升研究数据可得性；
- 完整的标注体系（belief state 与 dialogue act），支持从状态追踪到策略学习与生成的全流程研究；
- 详尽的数据收集与分析流程描述，可复现且便于扩展；
- 提供基线并设定统一评测框架，推动社区形成通用对比标准。

## 局限与讨论
- 众包对话仍可能存在不一致或标注偏差；
- 初始版本的标注质量与领域覆盖随后通过 2.x 系列版本逐步改进（如 MultiWOZ 2.1/2.2/2.4）；
- 任务型对话以结构化槽位为核心，面向开放域对话的泛化仍需其他数据与方法支持。

## 引用与链接
- arXiv：https://arxiv.org/abs/1810.00278
- PDF：https://arxiv.org/pdf/1810.00278
- DOI：https://doi.org/10.48550/arXiv.1810.00278
- 会议：EMNLP 2018（Accepted）

### BibTeX
```bibtex
@misc{budzianowski2018multiwoz,
  title        = {MultiWOZ -- A Large-Scale Multi-Domain Wizard-of-Oz Dataset for Task-Oriented Dialogue Modelling},
  author       = {Pawe\l Budzianowski and Tsung-Hsien Wen and Bo-Hsiang Tseng and Ilaria Casanueva and Stefan Ultes and Osman Ramadan and Milica Ga\si\'{c}},
  year         = {2018},
  eprint       = {1810.00278},
  archivePrefix= {arXiv},
  primaryClass = {cs.CL},
  doi          = {10.48550/arXiv.1810.00278},
  note         = {Accepted for publication at EMNLP 2018}
}
```
